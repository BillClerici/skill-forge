...
async def persist_scene_assignments_to_neo4j(state: Dict[str, Any], driver: Driver) -> None:
    ...
    try:
        ...
        logger.info(f"Processing {len(scene_assignments)} scene assignments")

        with driver.session() as session:
            relationships_created = 0

            for assignment_idx, assignment in enumerate(scene_assignments):
                scene_id = assignment.get("scene_id")
                scene_name = assignment.get("scene_name", f"Scene {assignment_idx}")

                if not scene_id:
                    logger.warning(f"Scene assignment {assignment_idx} missing scene_id, skipping")
                    continue
                ...
                try:
                    ...
                    logger.debug(f"Updating Scene {scene_id} metadata")
                    ...
                    logger.error(f"Failed to update Scene {scene_id} metadata: {str(e)}")
                ...
                for quest_obj_id in assignment.get("advances_quest_objectives", []):
                    try:
                        ...
                        logger.debug(f"Linking Scene {scene_id} to QuestObjective {quest_obj_id}")
                        ...
                        logger.error(f"Failed to link Scene {scene_id} to QuestObjective {quest_obj_id}: {str(e)}")
                        logger.error(f"Parameters: {params}")
                        # Non-critical, continue
                ...
                for campaign_obj_id in assignment.get("advances_campaign_objectives", []):
                    try:
                        ...
                        logger.debug(f"Linking Scene {scene_id} to CampaignObjective {campaign_obj_id}")
                        ...
                        logger.error(f"Failed to link Scene {scene_id} to CampaignObjective {campaign_obj_id}: {str(e)}")
                        logger.error(f"Parameters: {params}")
                        # Non-critical, continue
                ...
                for kg_spec in assignment.get("provides_knowledge", []):
                    ...
                    try:
                        ...
                        logger.debug(f"Linking Scene {scene_id} to Knowledge domain '{domain}' with params: campaign_id={campaign_id}")
                        ...
                        logger.error(f"Failed to link Scene {scene_id} to Knowledge domain '{domain}': {str(e)}")
                        logger.error(f"Parameters: {params}")
                        # Non-critical, continue
                ...
                for item_spec in assignment.get("provides_items", []):
                    ...
                    try:
                        ...
                        logger.debug(f"Linking Scene {scene_id} to Item category '{category}' with params: campaign_id={campaign_id}")
                        ...
                        logger.error(f"Failed to link Scene {scene_id} to Item category '{category}': {str(e)}")
                        logger.error(f"Parameters: {params}")
                        # Non-critical, continue
                ...
                if acquisition_methods:
                    try:
                        ...
                        logger.error(f"Failed to set acquisition methods for Scene {scene_id}: {str(e)}")
                        # Non-critical, continue
        ...
        logger.error(f"Scene assignments count: {len(state.get('scene_objective_assignments', []))}")
        raise
...
async def persist_redundancy_analysis_to_neo4j(state: Dict[str, Any], driver: Driver) -> None:
    ...
    try:
        ...
        campaign_id = state.get("final_campaign_id")
        if not campaign_id:
            logger.warning("No campaign_id found - skipping redundancy analysis")
            return

        logger.info(f"Using campaign_id: {campaign_id} for redundancy analysis")

        with driver.session() as session:
            # Analyze Knowledge redundancy
            try:
                params = {"campaign_id": campaign_id}
                logger.debug(f"Analyzing Knowledge redundancy with params: {params}")
                ...
                logger.error(f"Failed to analyze Knowledge redundancy: {str(e)}")
                logger.error(f"Parameters: campaign_id={campaign_id}")
                raise

            # Analyze Item redundancy
            try:
                params = {"campaign_id": campaign_id}
                logger.debug(f"Analyzing Item redundancy with params: {params}")
                ...
                logger.error(f"Failed to analyze Item redundancy: {str(e)}")
                logger.error(f"Parameters: campaign_id={campaign_id}")
                raise
            ...
            try:
                params = {"campaign_id": campaign_id}
                logger.debug(f"Getting redundancy counts with params: {params}")
                ...
                logger.error(f"Failed to get redundancy counts: {str(e)}")
                logger.error(f"Parameters: campaign_id={campaign_id}")
                # Non-critical, continue
